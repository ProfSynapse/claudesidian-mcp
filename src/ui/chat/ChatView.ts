...
export class ChatView extends ItemView {
    ...
    try {
      ...
      try {
        ...
        for await (const chunk of this.chatService.generateResponseStreaming(
          ...
          console.log(`[ChatView] Received token chunk: "${chunk.chunk}" (complete: ${chunk.complete})`);
          ...
          console.log(`[ChatView] Updating UI with accumulated content: "${streamedContent.substring(0, 50)}..." (${streamedContent.length} chars)`);
          ...
          if (chunk.complete) {
            console.log(`[ChatView] Streaming complete! Final content: ${streamedContent.length} characters`);
            
            // Save the final AI message to database
            console.log('[ChatView] Saving AI response to database...');
            await this.chatService.addMessage({
              conversationId: this.currentConversation.id,
              role: 'assistant',
              content: streamedContent
            });
            console.log('[ChatView] AI response saved to database');
  ...
  private updateStreamingMessage(messageId: string, content: string, isStreaming: boolean): void {
    console.log(`[ChatView] Updating streaming message ${messageId}, isStreaming: ${isStreaming}, content length: ${content.length}`);
    ...
    if (messageElement) {
      ...
      if (contentElement) {
        ...
        if (isStreaming) {
          ...
          console.log(`[ChatView] Set streaming content with cursor`);
        } else {
